...
namespace osseus {
    ...
    Vector3 UniversalElectroMag::CalculateForce(const OctNode &sourceNode, Handle handle, const BodyData &body) {
        ...
        if (farEnough && !(inBounds && sourceNode.ContainsBody(handle, queryPosition))) {
            Vector3 monopole =
                PointForce(sourceNode.GetCenterOfCharge(), sourceNode.GetTotalCharge(), queryPosition, queryCharge);

            Vector3 dipole =
                DipoleForce(sourceNode.GetDipoleMoment(), sourceNode.GetCenterOfCharge(), queryPosition, queryCharge);
    ...
    Vector3 UniversalElectroMag::DipoleForce(const Vector3 &dipoleMoment, const Vector3 &sourcePosition,
        ...
        const Vector3 field =
            (rHat * (3.0 * pDotR) - dipoleMoment) * (OsseusConstants::CoulombConstant / (distSq * dist));
