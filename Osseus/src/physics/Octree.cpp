...
namespace osseus {
    ...
    void OctNode::UpdateChargeProperties() {
        ...
        if (IsLeaf()) {
            // Accumulate charge and charge-weighted center
            for (const Entry& entry : entries_) {
        ...
        // Internal node: bottom-up accumulation from children
        for (const auto& child : children_) {
            if (!child) {
                continue;
            }
        ...
        if (std::abs(totalCharge_) > kChargeEpsilon) {
            ...
            // Fallback: geometric center of children
            Vector3 geometricCenter = Vector3::Zero();
            int childCount = 0;
            for (const auto& child : children_) {
                if (!child)
        ...
        // Accumulate dipole moment using parallel-axis theorem
        for (const auto& child : children_) {
            if (!child) {
                continue;
            }
