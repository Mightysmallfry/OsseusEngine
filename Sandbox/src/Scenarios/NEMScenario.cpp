#include "Scenarios/NEMScenario.h"
...
namespace sandbox {
    ...
    void NEMScenario::Initialize(osseus::PhysicsWorld& world, double width, double height) {
        boundaryRadius_ = std::min(width, height) / 2.0;

        // ==================== World

        world.AddUniversalForce(&emForce_);
        world.AddUniversalForce(&gravity_);
        ...
        std::bernoulli_distribution chargeSignDistribution(0.5);
        ...
        for (std::size_t i = 0; i < bodyCount; ++i) {
            ...
            const bool isPositive = chargeSignDistribution(generator);
            const double particleCharge = isPositive ? chargeMagnitude : -chargeMagnitude;

            const osseus::Vector3 position{randX, randY, 0.0};
            const osseus::Vector3 velocity{0.0, 0.0, 0.0};
            const osseus::BodyData body{position, velocity, mass, 1.0 / mass, particleCharge};
            ...
            particleShape->setFillColor(isPositive ? sf::Color::Cyan : sf::Color(255, 165, 0));
            renderObjects_.push_back({handle, std::move(particleShape)});
        }
    }

    void NEMScenario::Update(osseus::PhysicsWorld& world) {
